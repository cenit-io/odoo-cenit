...
class OmnaSyncProducts(models.TransientModel):
    ...
    # sync_type = fields.Boolean('Import By Integration', default=True)
    ...
    def import_function(self):
        try:
            # if self.sync_type:
                # record_id = self.env.ref('ecapi_mercado_libre.by_integration_import_products_cron').id
                # data = {'active': True,
            ...
            #     # record_id = self.env.ref('ecapi_mercado_libre.by_omna_id_import_products_cron').id
            #     # data = {'active': True,
            #     #         'nextcall': (datetime.now() + timedelta(minutes=2)).strftime('%Y-%m-%d %H:%M:%S'),
            #     #         'user_id': self.env.uid,
            ...
            #     record_id = self.env.ref('ecapi_mercado_libre.by_product_sku_import_products_cron').id
            #     data = {'active': True,
            #             'nextcall': (datetime.now() + timedelta(minutes=2)).strftime('%Y-%m-%d %H:%M:%S'),
            #             'user_id': self.env.uid,
