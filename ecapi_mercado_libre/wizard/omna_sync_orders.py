...
class OmnaSyncOrders(models.TransientModel):
    ...
    def sync_orders(self):
        try:
            ...
            if orders:
                ...
                self.env.user.notify_channel('warning', _("Sorry, we don't find results for this criteria. \n Please execute from Settings / Import Mercado Libre -> Cenit the option for import Orders, later try to execute this functionality."), _("Information"), True)
    ...
    def _create_orderline(self, line_item, currency):
        ...
        data = {
            ...
            'price_unit': product.product_tmpl_id.list_price if product else line_item.get('price'),
            ...
            'discount': 0,
            'product_template_id': product.product_tmpl_id.id if product else False,
            'route_id': False,
            # 'tax_id': [[6, False, [56]]],
        }
