    ...
    def import_function(self):
        try:
            if self.sync_type == 'by_integration':
                # record_id = self.env.ref('omna_mercado_libre.by_integration_import_products_cron').id
                # data = {'active': True,
            ...
            if self.sync_type == 'by_omna_id':
                # record_id = self.env.ref('omna_mercado_libre.by_omna_id_import_products_cron').id
                # data = {'active': True,
                #         'nextcall': (datetime.now() + timedelta(minutes=2)).strftime('%Y-%m-%d %H:%M:%S'),
                #         'user_id': self.env.uid,
            ...
            #     record_id = self.env.ref('omna_mercado_libre.by_product_sku_import_products_cron').id
            #     data = {'active': True,
            #             'nextcall': (datetime.now() + timedelta(minutes=2)).strftime('%Y-%m-%d %H:%M:%S'),
            #             'user_id': self.env.uid,
